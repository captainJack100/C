...
{ char *key, *dir, *arg;
  ...
  { struct api__doGoogleSearchResponse r;
    if (soap_call_api__doGoogleSearch(&soap, "http://api.google.com/search/beta2", "urn:GoogleSearchAction", key, arg, 0, 10, 1, "", 0, "", "latin1", "latin1", &r))
    { soap_print_fault(&soap, stderr);
      exit(-1);
    }
    else
    { int i;
      printf("documentFilteringi         = %d\n", (int)r._return.documentFiltering);
